...
func (h *Handler) Mount(mux *http.ServeMux, prefix string) {
	...
	mux.HandleFunc("GET "+prefix+"/new/recurring", h.recurringForm)
}
...
func (h *Handler) snapshotForm(w http.ResponseWriter, r *http.Request) {
	view, err := snapshots.BuildFormView(r.Context(), h.Client, r.URL.Query().Get("month"), false)
	if err != nil {
...
func (h *Handler) recurringForm(w http.ResponseWriter, r *http.Request) {
	view, err := h.accountPickerView(r)
	if err != nil {
		h.Logger.Error("build recurring action form", "err", err)
		http.Error(w, "failed to load recurring form", http.StatusBadGateway)
		return
	}
	h.renderFragment(w, "action_form_recurring", view)
}
