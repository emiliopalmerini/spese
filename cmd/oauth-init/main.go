...
func main() {
    ...
    case clientFile != "":
        ...
        if err != nil { log.Fatalf("read client file: %v", err) }
    default:
    ...
    if err != nil { log.Fatalf("oauth config: %v", err) }
    ...
    case code := <-codeCh:
        ...
        if err != nil { log.Fatalf("token exchange: %v", err) }
        // Save token
        outFile := os.Getenv("GOOGLE_OAUTH_TOKEN_FILE")
        if outFile == "" { outFile = "token.json" }
        f, err := os.OpenFile(outFile, os.O_CREATE|os.O_TRUNC|os.O_WRONLY, 0600)
        if err != nil { log.Fatalf("open token file: %v", err) }
        defer f.Close()
        if err := json.NewEncoder(f).Encode(tok); err != nil { log.Fatalf("write token: %v", err) }
